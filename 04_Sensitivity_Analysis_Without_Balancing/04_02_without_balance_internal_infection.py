...
"""
No-balancing internal TabNet model for infection prediction
...
DEFAULT_TARGET_COL = "culture_report"
OUTPUT_SUBDIR = "no_balance_internal_infection"

RAW_FEATURE_COLUMNS: List[str] = ['PFratio', 'GCS', 'Urine_output_8H', 'Temperature', 'Pulse', 'Respiration', 'SBP', 'WBC', 'CRP', 'PCT', 'Neutrophil_Seg', 'Ventilator_days', 'Antibiotics', 'Norepinephrine', 'Dopamine']

MODEL_FEATURE_COLUMNS: List[str] = ['min_PFratio', 'min_GCS', 'min_Urine_output_8H', 'max_Temperature', 'max_Pulse', 'max_Respiration', 'max_SBP', 'APACHEII_score', 'max_WBC', 'max_CRP', 'max_PCT', 'max_Neutrophil_Seg', 'max_Ventilator_days', 'max_Antibiotics', 'max_Norepinephrine', 'max_Dopamine']
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="No-balancing internal TabNet model for infection prediction"
    )
    parser.add_argument(
        "--train-input",
        required=True,
    ...
    # No external validation file is required for this internal-only script.
    parser.add_argument(
        "--target-col",
...
def main() -> None:
    ...
    # Internal-only script: no external validation is evaluated.
